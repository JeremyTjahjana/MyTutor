...
type TutorCardProps = {
  profile?: string | StaticImageData | null;
  name: string;
  role: string;
  description: string;
  rating: number;
...
const TutorCard = ({
  ...
  const displayRole =
    rolesArray.length > 4
      ? rolesArray.slice(0, 4).join(", ") + ", ..."
      : rolesArray.join(", ");

  return (
    <article className="flex w-[265px] shrink-0 min-h-[240px] flex-row items-stretch gap-3 rounded-2xl border border-[var(--gelap)]/15 bg-[var(--putih)] p-4 shadow-[0px_6px_0px_rgba(15,23,42,0.14),0px_10px_18px_rgba(15,23,42,0.16)] transition-transform duration-200 ease-out hover:-translate-y-1 active:translate-y-0 sm:w-full sm:max-w-[330px] sm:min-h-[290px] sm:gap-4 sm:p-5">
