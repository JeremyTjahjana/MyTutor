...
import { CheckCircle, Clock, XCircle, Loader2, CalendarCheck } from "lucide-react";
...
} from "@/app/server/actions/bookings.action";
...
function formatSchedule(start: string, end: string) {
  ...
  const startT = s.toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit", hour12: false });
  const endT   = e.toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit", hour12: false });
    ...
    <div className="bg-white rounded-xl border border-[var(--gelap)]/8 shadow-sm hover:shadow-md transition-all p-5 flex gap-4">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div>
            <p className="font-semibold text-[var(--gelap)] leading-tight">{booking.tutorName}</p>
            <p className="text-sm text-[var(--biru)] font-medium mt-0.5">{booking.subjectName}</p>
          </div>
        ...
        <p className="mt-2 text-sm text-[var(--gelap)]/60">
          🕐 {formatSchedule(booking.startTime, booking.endTime)}
        </p>
        {booking.notes && (
          <p className="mt-1.5 text-sm italic text-[var(--gelap)]/50">"{booking.notes}"</p>
        )}
...
export default function BookingsPage() {
  ...
  const handleAction = async (bookingId: string, action: "accept" | "cancel" | "complete") => {
  ...
  const todayBookings = bookings.filter((b) =>
    isToday(b.startTime) && (b.status === "pending" || b.status === "accepted")
  );
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold text-[var(--biru)] mb-1">Kelola Pemesanan</h1>
        <p className="text-[var(--gelap)]/60">Lihat dan kelola semua pesanan dari siswa.</p>
      </div>
      ...
      {todayBookings.length > 0 && (
        <section>
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="text-lg font-semibold text-[var(--biru)]">Jadwal Hari Ini</h2>
      ...
      {/* ── All Bookings ── */}
      <section>
        <h2 className="text-lg font-semibold text-[var(--gelap)] mb-3">Semua Pemesanan</h2>
        ...
        {filtered.length === 0 ? (
          <div className="bg-white rounded-xl border border-[var(--gelap)]/8 p-10 text-center">
            <p className="text-[var(--gelap)]/50">Tidak ada pemesanan di kategori ini.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {filtered.map((b) => (
